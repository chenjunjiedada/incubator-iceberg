...
public class EqualityDeleteRewriter implements Serializable {
  ...
  public List<DeleteFile> toPosDeletes(Pair<StructLike, CombinedScanTask> task) throws Exception {
    ...
    EqualityDeleteRowReader deleteRowReader = new EqualityDeleteRowReader(task.second(), schema, expectedSchema,
        nameMapping, io.value(), encryptionManager.value(), caseSensitive);
